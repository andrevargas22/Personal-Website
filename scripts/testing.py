...
_FORECAST_JSON_URL = (
    "https://raw.githubusercontent.com/andrevargas22/Energy_Consumption_AutoML"
    "/dev/data/plots/forecast_plot.json"
)


def fetch_energy_forecast_data() -> dict | None:
    """Fetch the latest energy forecast Plotly figure JSON from the AutoML repo.

    The JSON is generated by scripts/predict.py (Energy_Consumption_AutoML) and
    committed to the repository on every monthly update run.

    Returns:
        dict with 'data' and 'layout' keys ready for Plotly.newPlot(), or None
        if the request fails (the template renders a graceful fallback message).
    """
    try:
        resp = requests.get(_FORECAST_JSON_URL, timeout=15)
        resp.raise_for_status()
        return resp.json()
    except Exception as exc:
        logging.warning(f"[EnergyForecast] Could not fetch forecast data: {exc}")
        return None
